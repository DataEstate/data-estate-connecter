	...
	foreach ($deEstates as $asset) {
		...
		foreach ($templateKeys as $key) {
			...
			if (isset($asset->$key)) {
				...
				} else if ($key == "latest_date" || $key == "latest_end_date") {
					$curDate = new DateTime($asset->$key);
					if ($curDate->format('H') == 0 && $curDate->format('i') == 0) {
						$varArray[] = $curDate->format('D, d/m/y');
					} else {
						$varArray[] = $curDate->format('D, d/m/y h:i a');
					}
				} else {
