...
/***Shortcode For Description ****/	/***Shortcode For Address ****/
...
function dec_rate($atts, $content=null) {
	...
	if($error){
		...
		if (isset($api_arry->$rate)) {
			if ($type=="to") {
				return $api_arry->$rate->to;
			}
			else if ($type=="from") {
				return $api_arry->$rate->from;
			}
			else {
				return "";
			}
...
/***Shortcode For Email ****/	/***Shortcode For Image ****/
...
function dec_phone($atts, $content = null){
	extract(shortcode_atts(array('phones'=>'phones','index'=>'','get'=>''), $atts));
	global $api_arry;
	$error= api_error_function();
	if($error){
		//return $error;
		...
		if (isset($api_arry->{"phones"})) {
			if($index=='' && $get=='' ){
				return $api_arry->phones[0]->formatted_number;
			}
			else if($index!='' && $get==''){
				return $api_arry->phones[$index]->formatted_number;
			}
			else if($index=='' && $get!=''){
				$multi_get=explode(",",$get);
				$result_string = "";
